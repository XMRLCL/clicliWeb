<template>
    <div class="video-item" :style="initTheme()">
        <div class="img" @click="goVideo(info.vid, info.video)">
            <img :src="getResourceUrl(info.cover)" alt="封面" />
        </div>
        <div class="video-info">
            <span v-if="!props.keywords" class="title" @click="goVideo(info.vid, info.video)">{{ info.title }}</span>
            <span v-else class="title" @click="goVideo(info.vid, info.video)" v-dompurify-html="keyHighlight(info.title)"></span>
            <div class="author">
                <div class="avatar">
...
//前往视频详情
const goVideo = (vid: number, video: number) => {
    if (video == 1)
    {
      ...
      let videoUrl = router.resolve({ name: "Live", params: { vid: vid } });
      window.open(videoUrl.href, '_blank');
    }
}
