...
import { deleteResourceAPI, submitReviewAPI, submitflvkeyAPI, type BaseResourceType, type ResourceType } from "@leaf/apis";
import {videoflvkey} from "@leaf/apis/src/api/config";
...
const submitreview = () => {
  submitflvkeyAPI(videoflvkey, props.vid).then((res) => {
    if (res.data.code === statusCode.OK) {
      notification.success({
        title: '提交成功',
        duration: 5000,
